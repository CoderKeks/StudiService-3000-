...
class GraphicalUserInterface:    
    def __init__(self):
        ...
        self.root = tk.Tk(baseName=project_name, className=project_name, useTk=1)
    ...
    def show_main_menu(self):
        ...
        button_studierende = tk.Button(self.root, text="Studierende anzeigen und bearbeiten", command=self.show_studenten_list)
        button_studierende.config(font=('times', 15), justify='center', pady=5, padx=5, width=100, height=5)
        button_studierende.pack()

        button_kurse = tk.Button(self.root, text="Kurse anzeigen und bearbeiten", command=self.show_class_list)
    ...
    def show_studenten_list(self):
        ...
        create_btn = tk.Button(self.root, text="Neuen Studierenden anlegen", command=self.show_create_student)
        create_btn.pack(fill="x")

        scroll_bar = tk.Scrollbar(self.root)
        scroll_bar.pack( side = "right", fill="y")
        mylist = tk.Listbox(self.root, yscrollcommand = scroll_bar.set )
    ...
    def show_create_student(self):
        ...
        def handle_create_student():
            self.studierender_service.create(Studierender(name.get(), matrikelnummer.get(), studiengang.get()))
            self.show_studenten_list()

        submit_btn = tk.Button(self.root, text="Student anlegen",command=handle_create_student)
    ...
    def show_class_list(self):
        ...
        create_btn = tk.Button(self.root, text="Neuen Kurs anlegen", command=self.show_create_class)
        create_btn.pack(fill="x")
    ...
    def show_create_class(self):
        ...
        submit_btn = tk.Button(self.root, text="Kurs anlegen",command=handle_create_class)
        submit_btn.pack(pady=20)
    ...
    def add_home_btn(self):
        back_btn = tk.Button(self.root, text="Home", command=self.show_main_menu)
        back_btn.place(x=0, y=0)
        return back_btn

    def add_title(self, text):
        header = tk.Message(self.root, text=text)
    ...
    def add_field_with_label(self, text):
        ...
        tk.Entry(self.root, textvariable=label).pack()
        return label
